...
要解决这个——**我们在请求路径上挂一份"异步落日志"**(**调用日志 / LogStore**(每条 chat 调用记一行路径/时间/状态码/model,异步落盘,管理员通过 `/admin/logs` 和 `/admin/stats` 看)):中间件在 chat 端点返回 200 时把一行日志塞进内存缓冲(`_buffer`),后台 `flush_loop` 每 100ms 整段搬到落盘列表(`_flushed`),管理员通过 `/admin/logs` 和 `/admin/stats` 看。学完你看到每条调用"在飞 + 已落盘"两条痕迹:

1. **挂一个 `LogMiddleware`**。`@app.middleware("http")` 装在 s11 这层 app 上,包裹下面 s10 → s09 → ... 整条挂载链。`dispatch` 在 `call_next(request)` 拿到 `response` 后判断:`response.status_code == 200` 且路径以 `/v1/chat/completions` 结尾,就把 `{"path", "ts", "status", "model"}` 一行塞进 `log_store.enqueue`。中间件对所有 chat 调用一视同仁(不依赖具体 handler 实现),后续 s13 改了挂载结构也照样能看到——比在 chat handler 里直接 enqueue 更稳。只在 200 时记:4xx/5xx 不算"成功调用",s07 的配额结算失败、s08 的 429 限速、s10 的渠道故障,这些是另一类观测信号,留到 s16 才统一处理。
2. **中间件读 body 拿 model**。`model` 在请求体里、不在查询串里(Brief 原本写的是 `query_params.get("model")`,永远是 `"?"`——这是已知 bug)。中间件先 `await request.body()` 读出原始 bytes,`json.loads` 解出 `model` 写到 `request.state.model`,再用一个新的 `receive()` 把同一份 bytes 喂回去给下游 FastAPI。Starlette/FastAPI 下游 handler 需要重新读 body 才能拿到 `messages`、做转发;不重放就 422——所以必须重放,这是 Starlette 里读取并回放 body 的标准手法。Body 被读两次,小开销换干净的"中间件读 model"语义。
3. **`LogStore` Protocol + `InMemoryLogStore`**。`LogStore` 是 `typing.Protocol`(Python 的结构性子类型,只靠方法签名匹配),只有 `enqueue / list / reset / drain_now` 四方法。默认实现是 `InMemoryLogStore`:`_buffer: deque[dict]` 写入端 + `_flushed: list[dict]` 读取端 + `threading.Lock` 串两条路。抽 Protocol 的理由:(1) 测试可以用 duck-typed fake `set_default(rec)` 注入默认实例,验证中间件走的是注入路径;(2) v2 切 SQLite 时只换构造 `_default` 的语句,对外契约不变。日志条目来自异步中间件,但 `/admin/logs` 可能从同步线程读,deque+Lock 对两种调用方都安全;`asyncio.Queue` 跨线程就得 `run_coroutine_threadsafe`,对一个内存实现来说重了——所以用 deque+Lock 而非 asyncio.Queue。
4. **后台 `flush_loop` 每 100ms 搬运**。`flush_loop` 在 `@app.on_event("startup")` 启动,`while not stop_event: await asyncio.sleep(0.1); drain_now()`。`drain_now()` 把 `_buffer` 整段搬到 `_flushed`。单条 chat 调用几百 ms,瓶颈在等上游;落日志不能阻塞这条调用,100ms 批量 flush 把"每条调用写一次"的 IO 成本摊到"每 100ms 写一次"——所以异步搬不直接同步写。100ms 这个数字:够短让运营不会看到空日志、够长让 IO 开销可忽略。`@app.on_event("shutdown")` 里再同步 drain 一次,兼容 TestClient 退出 `with` 块时事件循环已停、最后一批不能留在 buffer 里。
