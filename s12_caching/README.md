...
同一段 prompt "给我讲个笑话"被 1000 个用户问时,上游会被叫 1000 次——账单按 token 翻倍,冷启动 800ms 也被 1000 份同样地付出去。客户端自带缓存做不到精确匹配,前端 JS 优化也救不了上游重复调用,必须由网关在 chat 路由外层包一道精确匹配闸门:同 model、同 messages、同 temperature 命中直接吐 bytes,短路所有下游,未命中照常转发,响应写回缓存。
