...
现在的场景是:`## 问题` 提的那件痛——s07 防住"算钱",但一个余额充足的用户仍可 1 秒打 100 次——只是把"速度维度"这件事的缺失暴露出来。真正的问题是:**撞到限速的那一刻,客户端只拿到 `429`,看不见"桶里还剩几枚令牌""还要等几秒才会补一枚""本次请求在哪个瞬间应该重试";而运维侧则只看得见"某个用户 429 了",看不见"他是突发用完还是持续低速触顶"**,更没法按这个桶给用户配套反馈。配额说"能花多少",我们还得说"能以这个速率花",而且说得**读得见**。这件事**没法靠"客户端自觉降速"或"配额收紧"能解决**,必须由网关在闸门之后插一条按用户的限速带,这件事才完整。
