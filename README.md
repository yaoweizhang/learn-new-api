...
一条 `/v1/chat/completions` 请求从客户端进入到上游返回，沿途经过鉴权（s05/s09） → 限速（s08） → 配额预扣（s07） → 缓存命中检查（s12） → 厂商适配派发（s04） → 可观测埋点（s11/s16） → 上游调用（s01-s04） → 配额结算（s07） → 失败回落到下一个渠道（s13） → 调用日志落盘（s11） → 响应回客户端。串起来 17 章是这台网关的"装配图"。
...
这是 new-api 的 Python 重写 + 精简版，删了：Go 适配器层类型反射、Redis 多节点共享、复杂的渠道组与赔率、计费规则引擎、完整的 RBAC。保留了：核心装配范式、预扣 + 结算双账号模型、按用户限速、可观测三件套（Prometheus + trace_id + log）。读者下一步可看：new-api 仓库的 `service/pre_consume.go`、`service/channel_select.go`、`middleware/RateLimit.go` —— 这三处对应 s07 / s13 / s08 的 Go 实现。
