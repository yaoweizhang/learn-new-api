...
客户端发请求时不知道这一笔要花多少 token,响应回来时如果上游 `usage` 缺失就拿不到账单——s05 那一段只做了透传,既不能在请求前给个报价,也没法在响应里给齐 `usage.prompt_tokens / completion_tokens / total_tokens`。客户端自己估会多扣少扣都救不回来,运维事后对账又跟不上,必须由网关在转发前先把 prompt 的 token 数清楚,再在响应阶段把上游的 `usage` 合并回来。
