...
要解决这个——**我们把"匿名 key 持有者"升级成"真用户"**:邮箱 + 密码注册、用 `bcrypt`(**专为密码哈希设计的慢哈希算法,反向暴力破解的成本极高**)存密码哈希、用 `HS256` **JWT / JSON Web Token**(把用户身份信息签名后塞进字符串,服务端不用查表就能验签,过期前一直可用)发"通行证"、再用这个通行证去访问 dashboard / admin。本章就做这一套:
...
s05 之前那张"API key → 用户"的内存表一旦对外公开就立刻撑不住——用户没法自己注册、key 明文存、进程一重启全部蒸发,这三件事客户端自己都搞不定,运维手工签发也搞不定,必须由网关把"匿名 key 持有者"升级成"真用户":邮箱注册 + bcrypt 存密码哈希 + HS256 签 JWT 当作"通行证"。
